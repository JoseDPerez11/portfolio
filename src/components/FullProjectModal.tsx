    ...
    return <Modal.Root scrollAreaComponent={ScrollArea.Autosize} size="70%" className="font-mono" opened={props.opened} onClose={props.close} >
        ...
        <Modal.Content className="!rounded-3xl">
            ...
            <Modal.Header className="!bg-bgColor !border-primaryColor !border-2 !border-b-0 !rounded-tl-3xl !rounded-tr-3xl" >
                <Modal.Title className="!text-4xl text-white flex gap-3 items-center font-bold" > {props.title}{props.live === true && <Badge className="flex items-center gap-1" size="lg" variant="outline" color="red" rightSection={<Indicator color="red" position="middle-end" size={10} processing></Indicator>} >Live</Badge>} </Modal.Title>
            ...
            <Modal.Body className="!bg-bgColor !pt-2 !border-primaryColor !border-2 !border-t-0 !rounded-bl-3xl !rounded-br-3xl" >
                ...
                <Group mt="md" mb="md" >
                    {props.technologies.map((tech: string, index: number, ) => index < 3 && <Badge key={index} size="xl" variant="!light" color="64FFDA"> {tech} </Badge>)}
                </Group>
